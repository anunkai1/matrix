...
def _reply(client: ChannelAdapter, chat_id: int, message_id: Optional[int], text: str) -> bool:
    client.send_message(
        chat_id,
        text,
...
def handle_voice_alias_command(
    ...
    if learning_store is None:
        return _reply(client, chat_id, message_id, "Voice alias learning is disabled.")

    pieces = raw_text.strip().split(maxsplit=1)
    tail = pieces[1].strip() if len(pieces) > 1 else ""
    if not tail or tail.lower() == "help":
        return _reply(client, chat_id, message_id, build_voice_alias_help_text())

    if tail.lower() == "list":
        pending = learning_store.list_pending()
        if not pending:
            return _reply(client, chat_id, message_id, "No pending learned voice alias suggestions.")
        ...
        return _reply(client, chat_id, message_id, "\n".join(lines))
    ...
    if approve_id is not None:
        ...
        if approved is None:
            return _reply(client, chat_id, message_id, f"No pending suggestion with id {approve_id}.")
        return _reply(
            client,
            chat_id,
            message_id,
    ...
    if reject_id is not None:
        ...
        if rejected is None:
            return _reply(client, chat_id, message_id, f"No pending suggestion with id {reject_id}.")
        return _reply(
            client,
            chat_id,
            message_id,
    ...
    if tail.lower().startswith("add "):
        ...
        if "=>" not in payload:
            return _reply(client, chat_id, message_id, "Usage: /voice-alias add <source> => <target>")
