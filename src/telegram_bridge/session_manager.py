...
def _send_worker_eviction_notice(client, chat_id: int) -> None:
    try:
        client.send_message(chat_id, WORKER_EVICTED_MESSAGE)
    except Exception:
        logging.exception("Failed to send worker-eviction notice for chat_id=%s", chat_id)


def _send_policy_refresh_notice(client, chat_id: int, message_id: Optional[int]) -> None:
    try:
        client.send_message(
            chat_id,
            WORKER_POLICY_REFRESH_MESSAGE,
...
def _ensure_chat_worker_session_canonical(
    ...
    return WorkerSessionEnsureOutcome(
        ...
        evicted_idle_chat_id=evicted_idle_chat_id,
...
def ensure_chat_worker_session(
    ...
    if outcome.evicted_idle_chat_id is not None and outcome.evicted_idle_chat_id in config.allowed_chat_ids:
        _send_worker_eviction_notice(client, outcome.evicted_idle_chat_id)
        emit_event(
            "bridge.worker_evicted_for_capacity",
            level=logging.WARNING,
            fields={
                "request_chat_id": chat_id,
                "evicted_chat_id": outcome.evicted_idle_chat_id,
            },
        )

    if outcome.session_replaced_for_policy:
        _send_policy_refresh_notice(client, chat_id, message_id)
        emit_event(
...
def expire_idle_worker_sessions(
    ...
    if state.canonical_sessions_enabled:
        ...
        emit_event(
            ...
            fields={
                "count": len(expired_chat_ids),
                "timeout_seconds": config.persistent_workers_idle_timeout_seconds,
                "canonical_mode": True,
            },
        )
...
def finalize_chat_work(
    ...
    emit_event(
        ...
        fields={"completed_chat_id": chat_id, "restart_chat_id": restart_chat_id},
    )
