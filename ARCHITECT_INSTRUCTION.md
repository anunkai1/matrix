...
- For these routine HA operations:
  ...
  - no mandatory `SERVER3_ARCHIVE.md` update per action
...
3. Ask for explicit user confirmation to proceed after the plan (for example: “Proceed with these changes?”), clearly state what approval is for (objective + exact scope/files), and WAIT for approval before implementing
...
Before making ANY change, Codex must print this plan format (for Objective - Use simple beginner language: explain what is being done and the high-level logical steps):
...
When execution is blocked pending approval, Codex must output this exact paused-state format before stopping:
