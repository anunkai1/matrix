...
- `sync.5.path`: `https://mavali.top/remote.php/dav/files/admin/Joplin`
- `sync.5.username`: `admin`
- `sync.5.password`: set locally at runtime (redacted, never committed)

## Apply / Rollback
- Apply: `bash ops/joplin/apply_server3.sh --url https://mavali.top --username admin --app-password '<secret>' --direction pull`
