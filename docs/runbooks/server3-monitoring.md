# Server3 Monitoring

## Purpose

Run a LAN-only Prometheus + Grafana + node_exporter stack for Server3 host monitoring.

## Live Shape

- compose root: `/srv/server3-monitoring`
- live stack file: `/srv/server3-monitoring/monitoring-stack.yml`
- live env file: `/etc/default/server3-monitoring`
- Grafana secret env file: `/etc/server3-monitoring/grafana-admin.env`
- systemd unit: `server3-monitoring.service`
- Grafana bind: `http://192.168.0.148:3000`
  - this is the current host-specific bind IP, not a permanent constant; if Server3's LAN IP changes, update `SERVER3_MONITORING_BIND_IP` and restart `server3-monitoring.service`
- Prometheus bind: `127.0.0.1:9090`
- node exporter: internal-only on the compose network
- external ARR metrics: exported through node_exporter textfile collector from `/var/lib/node_exporter_textfile/server3_external_arr.prom`
- external ARR exporter: root cron job from `/etc/cron.d/server3-external-arr-metrics`

## Credentials

- Grafana admin credentials are stored in `/etc/server3-monitoring/grafana-admin.env`.
- Keep that file `root:root` with mode `600`.
- Do not keep the live admin password in `/etc/default/server3-monitoring`.

## Live Env

Expected variables in `/etc/default/server3-monitoring`:

- `SERVER3_MONITORING_BIND_IP`
  - set this to Server3's current LAN IP; it can change over time
- `SERVER3_EXTERNAL_DISK_DF_PATH`
- `SERVER3_EXTERNAL_DISK_LABEL`
- `SERVER3_EXTERNAL_DISK_DF_CONTAINER`

Expected variables in `/etc/server3-monitoring/grafana-admin.env`:

- `GRAFANA_ADMIN_USER`
- `GRAFANA_ADMIN_PASSWORD`

Tracked secret example:

- `infra/env/server3-monitoring-grafana-admin.env.example`

Tracked non-secret example:

- `infra/env/server3-monitoring.env.example`

## Deployment Notes

- Grafana is the only exposed UI.
- Prometheus and node exporter are not exposed on the LAN.
- Grafana dashboards and datasource are provisioned from the repo.
- The external ARR disk metrics are generated by a root cron job that shells into a configured container path from `/etc/default/server3-monitoring`, runs `df`, and writes the result into the node_exporter textfile collector.
- The Grafana admin secret is intentionally split from the general monitoring env so the default monitoring env can stay limited to non-secret settings.
- Direct host-systemd and generic Docker namespace lookups of `/srv/external/server3-arr` were unreliable on this host and resolved against the root filesystem instead of the live external mount.
- Provisioned dashboards:
  - `Server3 Node Overview`
  - `Node Exporter Full` (`gnetId=1860`, revision `42`)
- `Server3 Node Overview` includes an external ARR section for `/srv/external/server3-arr`:
  - total bytes
  - used bytes
  - free bytes
  - used percent
  - `sdb` read/write throughput
- The systemd unit pre-creates/chowns the bind-mounted data directories before startup:
  - Grafana data: UID/GID `472:472`
  - Prometheus data: UID/GID `65534:65534`

## Basic Verification

```bash
sudo systemctl status server3-monitoring.service --no-pager
# replace the example IP with the current SERVER3_MONITORING_BIND_IP value if it has changed
curl -I http://192.168.0.148:3000/login
curl -s http://127.0.0.1:9090/api/v1/targets
```

## Files

- compose: `ops/server3_monitoring/monitoring-stack.yml`
- general env example: `infra/env/server3-monitoring.env.example`
- secret env example: `infra/env/server3-monitoring-grafana-admin.env.example`
- Prometheus config: `ops/server3_monitoring/prometheus/prometheus.yml`
- external ARR metric exporter: `ops/server3_monitoring/scripts/export_external_arr_metrics.sh`
- external ARR metric cron file: `infra/cron/server3-external-arr-metrics`
- Grafana provisioning: `ops/server3_monitoring/grafana/provisioning`
- dashboard JSONs:
  - `ops/server3_monitoring/grafana/dashboards/server3-node-overview.json`
  - `ops/server3_monitoring/grafana/dashboards/node-exporter-full.json`
- unit: `infra/systemd/server3-monitoring.service`
