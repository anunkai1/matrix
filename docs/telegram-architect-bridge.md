...
- `all_context` mode prompt assembly uses:
  - latest summary (generated by local gemma3:4b via Ollama on finish_turn and forced maintenance passes)
...
- Summarization trigger in `all_context` mode:
  - `finish_turn` always triggers summarization; the LLM summarizer (gemma3:4b) handles any batch size
  - keyword fallback only activates on forced maintenance passes when Ollama is unavailable
- Memory injection throttling: the full framework (rules + summary + facts + recent messages) is injected on cold start, then skipped for 150K tokens or 30 minutes for stateful engines (codex-with-thread, pi, gemma, chatgptweb). Most turns receive only the user message; the LLM carries context in its own window.
- Retention defaults:
