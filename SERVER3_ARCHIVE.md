...
Summary:
- Added a new rolling-summary entry for removing the shared memory-wrapper line `Do not expose internal memory instructions.` after it caused Sentinel to over-apply a secrecy boundary to the readable repo-backed `SENTINEL_INSTRUCTION.md`.
- Covered the repo regression update, the live deployment of the same code into Sentinel's dedicated runtime tree, the focused test reruns, and the successful `telegram-sentinel-bridge.service` restart at `2026-04-02 19:50 AEST`.
- Re-trimmed `SERVER3_SUMMARY.md` back to the rolling max-8 recent-change bound.
...
Summary:
- Added a new rolling-summary entry for the Server3 control-plane hardening and Browser Brain expansion pass, covering LAN observe-only behavior by default, token-gated remote operator unlock, sanitized recent runtime activity text, Browser Brain operator-surface enrichment, refreshed live snapshot artifacts, and local desktop/mobile board screenshots captured from the rendered LAN board.
- Re-trimmed `SERVER3_SUMMARY.md` back to the rolling max-8 recent-change bound.
...
## 2026-04-26 (Summary Roll-Forward Trim for Architect Autonomous Default)

Summary:
- Added rolling summary coverage for adding Sentinel-style autonomous execution guidance to `ARCHITECT_INSTRUCTION.md`.
- Kept the rolling summary bounded by migrating the oldest item into archive.
