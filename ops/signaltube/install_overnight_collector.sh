...
ASK_SERVICE_NAME="signaltube-lab-ask.service"
...
ASK_SERVICE_SRC="${REPO_ROOT}/infra/systemd/${ASK_SERVICE_NAME}"
...
ASK_SERVICE_DST="/etc/systemd/system/${ASK_SERVICE_NAME}"
TIMER_DST="/etc/systemd/system/${TIMER_NAME}"
ENV_DST="/etc/default/${ENV_NAME}"

if [[ ! -f "${SERVICE_SRC}" || ! -f "${RESCAN_SERVICE_SRC}" || ! -f "${ASK_SERVICE_SRC}" || ! -f "${TIMER_SRC}" || ! -f "${ENV_SRC}" ]]; then
...
case "${MODE}" in
  apply)
    ...
    sudo install -m 0644 "${ASK_SERVICE_SRC}" "${ASK_SERVICE_DST}"
  ...
  rollback)
    ...
    sudo rm -f "${TIMER_DST}" "${SERVICE_DST}" "${RESCAN_SERVICE_DST}" "${ASK_SERVICE_DST}"
    sudo systemctl daemon-reload
