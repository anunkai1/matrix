...
def runtime_docs(name: str) -> List[Tuple[str, str]]:
    ...
    return [("summary", "SERVER3_SUMMARY.md")]
...
def build_selected_runtimes(
    ...
    for key, source_names, role, operator_note in selected:
        ...
        rows.append(
            {
                "key": key,
                "name": "Govorun" if key == "govorun" else "Oracle" if key == "oracle" else live_rows[0]["name"],
                "stateClass": state_class,
                "stateText": state_text,
                "role": role,
                "operatorNote": operator_note,
                "summary": str(live_rows[0].get("purpose") or ""),
                "actions": action_labels("Govorun" if key == "govorun" else "Oracle" if key == "oracle" else str(live_rows[0]["name"])),
                ...
                "docsAndLogs": [{"label": label, "value": value} for label, value in runtime_docs("Govorun" if key == "govorun" else "Oracle" if key == "oracle" else str(live_rows[0]["name"]))],
