...
usage() {
  cat <<'USAGE'
Usage:
  server3-tv-brave-remote-debug-session.sh [url]

Examples:
  server3-tv-brave-remote-debug-session.sh
  server3-tv-brave-remote-debug-session.sh https://x.com/home
USAGE
}
...
BRAVE_PROFILE_DIR="${TV_HOME}/.local/state/server3-brave-remote-debug-profile"
...
echo "[server3-tv-brave-remote-debug-session] browser=brave url=${URL} remote_debugging_port=${REMOTE_DEBUG_PORT} profile=${BRAVE_PROFILE_DIR}"
