...
- 2026-03-05: hardened Govorun WhatsApp integration in repo with four reliability fixes: channel-aware memory namespacing (`tg:` vs `wa:`) in Python handlers/memory usage, pairing-code redaction from Node auth logs, Node bridge media-file retention controls (`WA_FILE_MAX_TOTAL_BYTES`, `WA_FILE_RETENTION_SECONDS`) with startup + periodic cleanup, and explicit API `400 invalid_json`/`413 request_too_large` handling; added regression tests in `tests/telegram_bridge/test_bridge_core.py` and updated WhatsApp env/runbook docs.
