...
Last updated: 2026-04-10 (AEST, +10:00)
...
- 2026-04-09: hardened shared YouTube link summarization in `src/telegram_bridge/handlers.py` and `ops/youtube/analyze_youtube.py` so default video summaries now separate `Source credibility:` from `Author reputation:` and perform a limited external reputation lookup using independent DuckDuckGo search results filtered away from YouTube/social links. The pipeline now fetches channel metadata from YouTube itself, fetches a small set of external reference results, feeds both into the model prompt, and appends deterministic fallback notes when the model omits them. Reputation now resolves to `no obvious red flags found in a limited external lookup`, `mixed`, or `unclear` instead of silently treating channel metadata as reputation. Added regression coverage in `tests/telegram_bridge/test_bridge_core.py` and `tests/youtube/test_analyze_youtube.py`, and verified with `python3 -m unittest tests.youtube.test_analyze_youtube tests.telegram_bridge.test_bridge_core -q` (`177` tests, passed).
