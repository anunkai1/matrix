...
- 2026-04-26: added the first Server3-side Gemma read-only agent harness. Gemma can now request bounded read-only tools through the bridge adapter for allowed file listing/reading, systemd service status, journal inspection, and a small exact allowlist of read-only commands; writes, deletes, restarts, installs, network changes, shell pipelines, and arbitrary commands remain blocked. Verified with 273 repo tests, a live `tank` user Gemma adapter read-file tool call against Server4, and a Tank bridge restart at `2026-04-26 10:53 AEST`; Tank restarted active with `Engine plugin active=gemma`, though the restart interrupted one in-flight Tank request and the startup recovery marked it processed.
