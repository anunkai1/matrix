...
- 2026-03-08: hardened Govorun policy enforcement by anchoring `executor.sh` to `/home/govorun/govorunbot`, making `AGENTS.md` resolution absolute, adding startup fingerprint-based reset for stored non-worker chat threads when watched policy files change, setting live `TELEGRAM_POLICY_WATCH_FILES=/home/govorun/govorunbot/AGENTS.md`, and verifying Iran/current-affairs prompts now decline on the live executable path instead of answering politics from stale context.
