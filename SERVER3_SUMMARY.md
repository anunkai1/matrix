...
- 2026-03-27: added deterministic Architect planner preflight tooling (`python3 ops/telegram-bridge/planner_preflight.py`), tightened worker-lane keyword matching to avoid accidental substring-triggered docs/verification lanes, added machine-readable planner reason codes and candidate/selected-role logging, and made the drain-aware restart helper persist one durable pass/fail/timeout status marker per unit under `/run/restart-and-verify/restart_and_verify.<unit>.status.json` so bridge-triggered restarts stay visible even with `PrivateTmp=true`.
...
- 2026-03-27: hardened the shared Telegram bridge restart helper to wait for persisted in-flight work to clear before service restart, using canonical session state when enabled, and fixed the Architect split-planner worker helper so temporary read-only planner runs no longer fail on closed-stdin subprocess handling.
