...
- 2026-03-18: verified that the live Browser Brain runtime cannot run in headed/visible mode on Server3 as currently provisioned because `lightdm.service` is inactive, `$DISPLAY` is unset, and no `Xvfb` binary is installed; Browser Brain was left in headless mode so the managed profile at `/var/lib/server3-browser-brain/profile` remains usable for normal automation, and any future manual-login-on-screen path will require adding a real display/X server attachment strategy first.
