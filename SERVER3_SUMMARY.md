...
- 2026-03-18: deployed Server3 Browser Brain on the live host as the dedicated `browser_brain` runtime, enabled `server3-browser-brain.service`, validated live start/open/snapshot/click browser control against `example.com`, and switched the runtime backend to Playwright persistent-context launch plus a single-threaded HTTP server after live smoke tests exposed a hanging CDP bootstrap and Playwright thread-affinity issue.
