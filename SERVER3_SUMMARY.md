...
- 2026-04-26: added the first Server3-side Gemma read-only agent harness. Gemma can now request bounded read-only tools through the bridge adapter for allowed file listing/reading, systemd service status, journal inspection, and a small exact allowlist of read-only commands; writes, deletes, restarts, installs, network changes, shell pipelines, and arbitrary commands remain blocked. Follow-up fixed Tank/Gemma returning `(No output from TANK)` for broad "list files you can read" prompts by adding a deterministic read-only listing fallback and tightening file disclosure guards so `.git`, `private`, virtualenv/cache dirs, and sensitive-looking names are not listed/read. Verified with 275 repo tests, live `tank` user Gemma adapter read-file and list-files calls against Server4, and Tank bridge restarts at `2026-04-26 10:53 AEST` and `2026-04-26 11:04 AEST`; Tank is active with `Engine plugin active=gemma`.
