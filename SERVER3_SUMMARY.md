...
- 2026-03-31: upgraded the Server3 control-plane sketch into a browser-local read-only operator page by adding `ops/server3_control_plane/export_snapshot.py` plus `ops/server3_control_plane/refresh_snapshot.sh`, generating `docs/server3-control-plane-data.{json,js}` from live Server3 status/timer/host data, and rewiring `docs/server3-control-plane-sketch.html` to consume that snapshot directly from `file://` while preserving the mission-control runtime rail, activity spine, approvals inbox, jobs lane, and host/storage floor.
