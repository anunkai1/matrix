...
- 2026-04-13: removed Architect's remaining default link-summary source-analysis preamble by updating local `private/SOUL.md` so pasted YouTube/article summaries stay summary-first unless the owner explicitly asks for extra source vetting, and logged the correction in `LESSONS.md`.
- 2026-04-13: removed the default YouTube source-analysis injection end to end for Sentinel by aligning the stale dedicated runtime copy of `src/telegram_bridge/handlers.py` with the current shared repo version, updating the bridge regression test to keep the summary prompt limited to the actual video content, and then restarting `telegram-sentinel-bridge.service` for the live fix.
