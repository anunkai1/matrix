...
- Dream-loop operator visibility: `/truth_status` now surfaces claim-verification mode and claim-drift summary from the nightly dream-loop truth baseline, explicit truth-changed/delivery/reset status for the current stale-warning fingerprint, unresolved items, and one short global system line; the first-wave executable dream-loop claim set covers both `ARCHITECT_INSTRUCTION.md` and claim-backed `SERVER3_SUMMARY.md` verification, the loop keeps an append-only `history.jsonl` audit trail under the dream-loop state directory, safe repo-managed dream-loop outputs can now auto-commit/push without sweeping up pre-existing staged or dirty work, and eligible scopes now get one stale-context warning per truth-change fingerprint until `/reset` or a later fingerprint supersedes it.
