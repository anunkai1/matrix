...
Last updated: 2026-04-23 (AEST, +10:00)
...
- 2026-04-23: added a tracked Home Assistant daily AC schedule on Server3 for `climate.mid_kids_rm_aircon` and `climate.living_rm_aircon`. New repo runner `ops/ha/run_daily_ac_schedule.sh` plus systemd units `ha-daily-ac-schedule@.service` and timers `ha-daily-ac-schedule-{midnight-heat,morning-heat,morning-off}.timer` now enforce: mid room `heat 22C` at `23:50` AEST, both mid room and living `heat 26C` at `05:30` AEST, and both off at `09:00` AEST. Installed live under `/etc/systemd/system` and enabled with next triggers verified for 2026-04-23/24.
...
- 2026-04-20/21: updated Foodle in `docs/projects/foodle.html` so the combined calendar starts weeks on Monday, share text includes the public URL, the mobile tracker layout keeps the compact Fasting and Coffee cards side by side instead of making Fasting span the full row, and the calendar/trackers now support a third red dot-style `Over eating` marker alongside Fasting and Coffee; the live Server2 Foodle HTML was republished after each change.
