...
- 2026-05-09: added a low-agency cooldown rule to Ralph target selection. Repeated high-score `codex_exec_latency` passes are now temporarily de-ranked after 3 consecutive selections, so execute mode can move on to more locally fixable targets like `progress_edit_noise` while still reporting the raw Codex latency telemetry.
