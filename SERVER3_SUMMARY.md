...
- 2026-04-09: hardened shared YouTube link summarization in `src/telegram_bridge/handlers.py` so default video summaries now explicitly include a compact but informative `Source credibility:` explanation instead of a bare label, and added a YouTube-only post-processing fallback that appends the note when the model omits it; the note now explains creator format, commentary vs primary reporting, and transcript-quality caveats from metadata/transcript source. Added regression coverage in `tests/telegram_bridge/test_bridge_core.py` and verified with `python3 -m unittest tests.telegram_bridge.test_bridge_core -q` (`166` tests, passed).
