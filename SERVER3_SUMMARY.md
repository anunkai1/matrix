...
- 2026-03-04: added runtime observer daily-digest capability with new modes `telegram_daily_summary` and `telegram_alerts_daily`, local-hour scheduling (`RUNTIME_OBSERVER_DAILY_SUMMARY_HOUR_LOCAL`, default `20`), window sizing (`RUNTIME_OBSERVER_DAILY_SUMMARY_WINDOW_HOURS`, default `24`), and summary lines that include warn/critical occurrence counts per KPI.
