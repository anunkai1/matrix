...
- 2026-03-31: upgraded the Server3 control-plane sketch into a browser-local operator surface by adding snapshot generation (`ops/server3_control_plane/export_snapshot.py`, `refresh_snapshot.sh`) plus a localhost API server (`ops/server3_control_plane/serve.py`, `run_local.sh`), generating `docs/server3-control-plane-data.{json,js}` from live Server3 status/timer/host data, and rewiring `docs/server3-control-plane-sketch.html` to work both as a `file://` snapshot page and as a served localhost UI with working refresh, restart, and live-log actions.
