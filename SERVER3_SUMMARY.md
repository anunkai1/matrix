...
- Dream-loop operator visibility: `/truth_status` now surfaces claim-verification mode and claim-drift summary from the nightly dream-loop truth baseline, and `SERVER3_SUMMARY.md` verification is backed by an executable claim registry rather than only hard-coded line mappings.
