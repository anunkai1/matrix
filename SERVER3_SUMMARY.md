...
- Core capabilities: text/photo/voice/document handling, simplified per-chat recent-message memory (last ~5000 tokens verbatim), optional persistent workers, optional canonical session model, safe queued `/restart`.
...
- Architect memory is now simplified: only recent messages (last ~5000 tokens) are injected verbatim into the prompt. No summaries, facts, context rules, or shared archive. `/reset` clears chat messages. No nightly merge timer.
...
- Memory is simplified: only recent messages (last ~5000 tokens verbatim) are injected. No summaries, facts, context rules, unsummarized tail, or shared archive. `/reset` clears chat messages. No nightly merge timer. Memory code reduced from ~2100 to ~400 lines.

## Recent Changes (Rolling Max 8)
- 2026-05-04: simplified memory system. Removed conversation summaries, durable facts, memory context rules, unsummarized tail, nightly archive merge, and all associated code (`memory_summary_utils.py`, `llm_summarizer.py`). Prompt now injects only recent messages (last ~5000 tokens) + current user input. Memory engine reduced from ~2100 to ~400 lines. Removed merge timer units.
