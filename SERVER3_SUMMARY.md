...
- 2026-04-13: removed Sentinel's last dedicated-runtime YouTube reputation lookup from `/home/sentinel/sentinelbot/ops/youtube/analyze_youtube.py`; the live analyzer no longer performs DuckDuckGo author-reputation fetches or emits `external_reputation`, and direct verification against a sample video still returned `automatic_captions` successfully.
- 2026-04-13: removed Architect's remaining default link-summary credibility injection by updating local `private/SOUL.md` so pasted YouTube/article summaries no longer prepend `Source credibility` notes unless the owner explicitly asks for source evaluation, and logged the correction in `LESSONS.md`.
