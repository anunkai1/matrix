...
- 2026-04-19: added Ask SignalTube conversational scan mode. Server2 now renders an authenticated `Ask SignalTube` prompt box and proxies `POST /signaltube/api/ask` to Server3; Server3 control plane exposes authenticated `POST /api/signaltube/ask` and starts `signaltube-lab-ask.service`, which interprets the prompt against the previous Ask session, scans generated YouTube queries through Browser Brain, replaces visible results with an `Ask SignalTube` feed, and pushes/rerenders Server2. Current implementation uses a local language planner for topic/refinement/viewpoint parsing; no external LLM key is configured yet.
