...
- 2026-03-31: hardened the deployed Server3 control-plane LAN surface so `docs/server3-control-plane-sketch.html` now presents live LAN view mode while keeping refresh/log/restart operator actions local-only on `localhost`, `ops/server3_control_plane/serve.py` now rejects remote refresh/log/restart calls with `403`, `ops/server3_control_plane/export_snapshot.py` now prefers privileged journal reads and summarizes recent runtime events instead of dumping raw structured log payloads, regenerated `docs/server3-control-plane-data.{json,js}`, re-verified the live unit after restart, and captured fresh desktop/mobile screenshots from the rendered LAN board locally instead of routing previews through Telegram.
