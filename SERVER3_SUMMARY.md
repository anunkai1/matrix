...
- 2026-04-26: added a selectable Pi engine path to the shared Telegram bridge. The corrected `pi` design runs the Pi binary locally inside each Server3 runtime root and uses Server4 Beast only as the Ollama model backend through an SSH tunnel; configurable knobs include `PI_RUNNER`, `PI_LOCAL_CWD`, `PI_PROVIDER`, `PI_MODEL`, `PI_TOOLS_MODE`, and timeout/tunnel settings. `/engine status` reports Pi health/version/model availability. Verified focused bridge tests and direct Tank local Pi checks.
...
- 2026-04-26: made Tank the first default Gemma/Pi test runtime. Live `/etc/default/telegram-tank-bridge` now sets `TELEGRAM_ENGINE_PLUGIN=gemma`, `TELEGRAM_SELECTABLE_ENGINE_PLUGINS=codex,gemma,pi`, explicit Server4 Gemma env (`GEMMA_PROVIDER=ollama_ssh`, `GEMMA_MODEL=gemma4:26b`, `GEMMA_SSH_HOST=server4-beast`, `GEMMA_REQUEST_TIMEOUT_SECONDS=180`), and local Pi runner env (`PI_RUNNER=local`, `PI_LOCAL_CWD=/home/tank/tankbot`, tunnel port `11435`). Restart verification passed at `2026-04-26 10:21:48 AEST`; Tank logs now report `Engine plugin active=gemma`, and direct `tank` user adapter checks returned `tank default gemma ok` and local Pi identity/cwd as Tank in `/home/tank/tankbot`.
