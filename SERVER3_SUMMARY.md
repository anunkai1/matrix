...
- Runtime pattern: Telegram long polling + live `codex app-server` for Architect Codex turns, plus live per-scope Pi RPC sessions for `/engine pi`, with same-scope plain-text follow-up steering while those live turns are active
...
- Goal operator visibility: `/goal status` now reports the goal lifecycle for the current scope together with canonical-session linkage, worker thread attachment, and in-flight request state; goal continuations also support a dedicated configurable judge engine/model route and configurable turn budget.
- Dream-loop operator visibility: `/truth_status` now surfaces claim-verification mode and claim-drift summary from the nightly dream-loop truth baseline, explicit truth-changed/delivery/reset status for the current stale-warning fingerprint, unresolved items, and one short global system line; the executable dream-loop claim set now covers bounded `ARCHITECT_INSTRUCTION.md` time/identity/execution-policy/session-start claims together with claim-backed `SERVER3_SUMMARY.md` verification, the loop keeps an append-only `history.jsonl` audit trail under the dream-loop state directory, safe repo-managed dream-loop outputs can now auto-commit/push without sweeping up pre-existing staged or dirty work, and eligible scopes now get one stale-context warning per truth-change fingerprint until `/reset` or a later fingerprint supersedes it.
- Follow-up steering: active Codex app-server turns support same-scope plain-text follow-up steering across direct chats, groups, and forum topics, with short coalescing to fold nearby follow-up messages together; active Pi scope sessions now also accept same-scope plain-text follow-ups and merge them before the bridge sends the final reply.
...
- 2026-05-26: `/goal` now supports a dedicated configurable goal-judge engine/model path plus configurable goal turn budgets, closing the main remaining Hermes parity gap in the goal loop.
- 2026-05-26: dream loop now uses an executable claim registry with persisted `claim_results` / `claim_summary` / `stale_claims`, and `SERVER3_SUMMARY.md` verification/correction for approved bullets is claim-backed instead of raw mapped-field alignment.
- 2026-05-26: dream loop first-wave claim coverage now includes bounded `ARCHITECT_INSTRUCTION.md` claims in addition to the approved `SERVER3_SUMMARY.md` claims, so the initial claim surfaces from the spec are both active.
- 2026-05-26: dream loop now appends a bounded per-run `history.jsonl` audit trail so alignment, claim drift, stale-context changes, and git automation outcomes remain inspectable beyond the latest-state snapshot.
- 2026-05-26: dream loop `v2.1` git automation now auto-commits and pushes only safe current-run repo-managed outputs, while explicitly skipping pre-existing staged changes and pre-run dirty managed files.
- 2026-05-26: the bridge now delivers one-shot stale-context warnings for scopes marked by the dream loop, and suppresses repeats until `/reset` or a new warning fingerprint is recorded.
- 2026-05-26: `/truth_status` now shows whether truth changed on the last run, whether the current stale-warning fingerprint was already delivered or cleared with `/reset`, unresolved items, and one compact global system line.
- 2026-05-26: dream loop broadened bounded `ARCHITECT_INSTRUCTION.md` claim coverage to include execution-policy and session-start-checklist claims in addition to the earlier time/service identity claims.
