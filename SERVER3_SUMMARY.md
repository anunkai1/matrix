...
- 2026-03-07: fixed Oracle Signal identity persistence bugs by making memory reset actually delete conversation messages/facts/summaries (not just the thread row), removing the hard-coded `Architect` assistant label from memory writes, and setting an Oracle-specific `TELEGRAM_RESPONSE_STYLE_HINT` so fresh Signal chats identify as Oracle after `/reset`.
