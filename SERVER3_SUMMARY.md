...
- 2026-04-26: made Tank the first default Gemma test runtime. Live `/etc/default/telegram-tank-bridge` now sets `TELEGRAM_ENGINE_PLUGIN=gemma`, `TELEGRAM_SELECTABLE_ENGINE_PLUGINS=codex,gemma,pi`, explicit Server4 Gemma env (`GEMMA_PROVIDER=ollama_ssh`, `GEMMA_MODEL=gemma4:26b`, `GEMMA_SSH_HOST=server4-beast`, `GEMMA_REQUEST_TIMEOUT_SECONDS=180`), and `PI_REMOTE_CWD=/home/v/pi-bridge-workspaces/tank`. Restart verification passed at `2026-04-26 10:21:48 AEST`; Tank logs now report `Engine plugin active=gemma`, and direct `tank` user adapter checks returned `tank default gemma ok` and `/home/v/pi-bridge-workspaces/tank`.
