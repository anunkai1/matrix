...
- 2026-03-05: added cross-channel media URL transcription (`/yt`, `/yt-transcribe`, `/transcribe-url`) in `src/telegram_bridge/handlers.py` with WhatsApp prefix-bypass for these commands, new backend script `ops/telegram-voice/transcribe_media_url.sh` (captions-first then local Whisper fallback), plus docs/test updates so Telegram, WhatsApp, and CLI can use the same pipeline.
