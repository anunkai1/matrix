...
- Dream-loop operator visibility: `/truth_status` now surfaces claim-verification mode and claim-drift summary from the nightly dream-loop truth baseline, the first-wave executable dream-loop claim set covers both `ARCHITECT_INSTRUCTION.md` and claim-backed `SERVER3_SUMMARY.md` verification, and the loop now keeps an append-only `history.jsonl` audit trail under the dream-loop state directory.
