...
- Dream-loop operator visibility: `/truth_status` now surfaces claim-verification mode and claim-drift summary from the nightly dream-loop truth baseline, and the first-wave executable dream-loop claim set now covers both `ARCHITECT_INSTRUCTION.md` and claim-backed `SERVER3_SUMMARY.md` verification.
