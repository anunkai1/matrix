...
- Server4 Gemma integration keeps Server3 as the bot/control-plane host: use `/engine gemma`, `/engine codex`, `/engine reset`, and `/engine status` per chat/topic; current Gemma path is text-only with live `/engine status` health detail, while the experimental read-only/web agent harness has been rolled back.

## Recent Changes (Rolling Max 8)
- 2026-04-26: added Sentinel-style `1A) Autonomous Default` guidance to `ARCHITECT_INSTRUCTION.md`. Architect now explicitly continues clear scoped tasks through investigation, editing, testing, logging, and verification without routine confirmation, while still stopping for destructive/irreversible actions, ambiguous targets, scope expansion, secrets/credentials/payments/security-sensitive work, or repeated failures requiring a decision.
