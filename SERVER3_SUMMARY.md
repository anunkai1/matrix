...
- 2026-04-26: added an experimental `ops/chatgpt_web_bridge.py` CLI that treats `chatgpt.com` as a brittle Browser Brain-backed whole-brain oracle for lab use. It opens/reuses a ChatGPT tab through Browser Brain, detects login/CAPTCHA/blocking states, submits a prompt, and extracts a best-effort response from ARIA snapshots. This is intentionally CLI-first and parallel to existing Codex/Gemma/Pi engines; it does not change live Telegram routing.
