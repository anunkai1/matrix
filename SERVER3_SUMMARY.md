...
- 2026-03-18: replaced the Govorun WhatsApp 09:00 daily message rotation with a local Reddit-backed cache of highly popular `r/LifeProTips` posts from the last 5 years plus persistent anti-repeat SQLite history, and updated the sender/runbook/env template so the delivered message preserves the full cached source advice in Russian without adding a Reddit/source link.
- 2026-03-18: deployed Server3 Browser Brain on the live host as the dedicated `browser_brain` runtime, enabled `server3-browser-brain.service`, validated live start/open/snapshot/click browser control against `example.com`, switched the runtime backend to Playwright persistent-context launch plus a single-threaded HTTP server after live smoke tests exposed a hanging CDP bootstrap and Playwright thread-affinity issue, and added the first Architect caller path via `Server3 Browser ...` / `Browser Brain ...` plus the local `ops/browser_brain/browser_brain_ctl.sh` wrapper.
