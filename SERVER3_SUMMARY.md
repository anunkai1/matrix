...
- `Server3 Ralph Autopilot` now runs as a repo-backed hourly optimization executor (`server3-ralph-loop.timer`) plus a daily Telegram Markdown reporter (`server3-ralph-daily-report.timer`). Its live `execute` path in `ops/ralph_loop/ralph_loop.py` can select a backlog target, run one Codex optimization pass, verify it, commit/push successful work, record `execution_results.jsonl`, and then re-rank from fresh telemetry.
...
- 2026-05-10: added English TTS voice replies via `ops/telegram-voice/tts_english.sh`. Uses Microsoft Edge TTS (free, no API key) with ffmpeg for OGG Opus conversion. Supports speed control via `TTS_SPEED` env var (default 1.35x). Assistant can reply to voice messages with voice notes using the bridge's existing `sendVoice` outbound media pipeline with `[[media: file.ogg]] [[audio_as_voice]]` directives.
...
- 2026-05-09: updated Ralph execute mode so successful runs commit and push their own verified changes, while commit/push is now skipped and reported when a run touches files that were already dirty before execution.
- 2026-05-09: named the loop canonically as `Server3 Ralph Autopilot`, changed dirty-worktree handling so Ralph still runs and reports findings instead of refusing, added per-run Telegram updates, and added a daily Markdown report timer. Live daily delivery is now centralized through `staker_alerts_bot` to chat `211761499`.
- 2026-05-09: removed `codex_exec_latency` from Ralph execute-mode selection. Ralph still reports that raw latency issue in telemetry, but it no longer spends autonomous optimization passes trying to fix an owner-accepted upstream-dominated bottleneck.
