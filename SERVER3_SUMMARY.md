...
- 2026-04-26: added an experimental `ops/chatgpt_web_bridge.py` CLI that treats `chatgpt.com` as a brittle Browser Brain-backed whole-brain oracle for lab use. It opens/reuses a ChatGPT tab through Browser Brain, detects login/CAPTCHA/blocking states, submits a prompt, now prefers the latest ChatGPT `Copy` button plus Browser Brain clipboard read for response extraction, and falls back to ARIA snapshot parsing. This is intentionally CLI-first and parallel to existing Codex/Gemma/Pi engines; it does not change live Telegram routing.
