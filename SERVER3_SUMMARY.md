...
- 2026-03-04: enabled Govorun WhatsApp voice-note transcription by wiring live `/etc/default/govorun-whatsapp-bridge` with `TELEGRAM_VOICE_TRANSCRIBE_CMD` + dedicated whisper runtime env (`TELEGRAM_VOICE_WHISPER_VENV`, socket/log path, `HF_HOME` cache reuse) and setting `TELEGRAM_VOICE_WHISPER_MODEL=medium`; voice-prefix enforcement now silently ignores non-prefixed WhatsApp transcripts after transcription while still executing prefixed transcripts.
