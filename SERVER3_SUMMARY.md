...
Last updated: 2026-04-03 (AEST, +10:00)
...
- 2026-04-03: added a standalone Android-first child-shell prototype at `docs/projects/kids-world-prototype.html` that makes the proposed Kids World architecture concrete as a single-file interactive web app, covering the bounded home shell, guided Create room, Learn round, Missions reward economy, and approved Watch shelf without touching live runtime services.
...
- 2026-04-03: fixed `docs/projects/foodle.html` day persistence again by removing the destructive fallback that deleted the prior local-date record whenever a “yesterday” check-in was saved, which in AEST could erase the previous marker and make it look like the entry moved forward one day; `streakFor()` now also anchors from the most recent logged day before counting, so an overnight rollover no longer resets the visible streak before the next check-in is entered.
