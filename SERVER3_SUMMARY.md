...
- Server4 Gemma integration keeps Server3 as the bot/control-plane host: use `/engine gemma`, `/engine codex`, `/engine reset`, and `/engine status` per chat/topic; current Gemma path is text-only, has bounded Server3 read-only tools, and Tank now has raw public web research mode through the Server3 gateway.

## Recent Changes (Rolling Max 8)
- 2026-04-26: added raw public web research mode for Gemma through the Server3-side read-only harness. When `GEMMA_WEB_RESEARCH_ENABLED=true`, Gemma can request `web_search(query, max_results)` and `fetch_url(url, max_bytes)` for arbitrary public `http`/`https` targets; the gateway still blocks local/private/loopback/link-local/multicast/reserved/unspecified network targets so it cannot probe Server3, Server4, or LAN services. Tank live env now enables the mode, Tank was restarted and verified active at `2026-04-26 11:48 AEST`, direct Tank-user adapter smoke fetched `https://example.com`, and a direct web search for `example domain` returned public results. Follow-ups fixed empty responses for bare-domain prompts like `Can you search headlines from rt.com` by routing headline/source requests to `site:<domain> headlines` search and filtering DuckDuckGo ad redirects, then fixed `Look for AI news and report headlines` by routing look-for/find/get/report news/headline prompts to web search. User explicitly accepted all risk for Tank/Gemma full sudo lab mode; live Tank now has `/etc/sudoers.d/tank-gemma-full-sudo`, `GEMMA_DANGEROUS_SUDO_ENABLED=true`, and Gemma tool `run_sudo_command(command)`, verified with Tank-user adapter smoke `sudo id` returning `uid=0(root)` at `2026-04-26 13:46 AEST`. Initial verification: 548 full tests with `--import-mode=importlib`; latest follow-up verification: 189 focused Gemma/bridge tests and Ruff on touched Python files.
- 2026-04-26: added the first Server3-side Gemma read-only agent harness. Gemma can now request bounded read-only tools through the bridge adapter for allowed file listing/reading, systemd service status, journal inspection, and a small exact allowlist of read-only commands; writes, deletes, restarts, installs, network changes, shell pipelines, and arbitrary commands remain blocked. Follow-up fixed Tank/Gemma returning `(No output from TANK)` for broad "list files you can read" prompts by adding a deterministic read-only listing fallback and tightening file disclosure guards so `.git`, `private`, virtualenv/cache dirs, and sensitive-looking names are not listed/read. Second follow-up made fallback intent use only the current user message, added deterministic `read_file` fallback for "post/read/show what's inside <file>" prompts, and supports case-insensitive filename resolution for files such as `agents.md` -> `AGENTS.md`. Verified with 276 repo tests, live `tank` user Gemma adapter read-file and list-files calls against Server4, and Tank bridge restarts at `2026-04-26 10:53 AEST`, `2026-04-26 11:04 AEST`, and `2026-04-26 11:10 AEST`; Tank is active with `Engine plugin active=gemma`.
...
- Tank/Gemma full-sudo lab mode is intentionally dangerous and currently live: `/etc/sudoers.d/tank-gemma-full-sudo` gives `tank` passwordless root `ALL`, and `/etc/default/telegram-tank-bridge` enables `GEMMA_DANGEROUS_SUDO_ENABLED=true`. Disable/remove both when the research window closes.
