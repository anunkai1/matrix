...
class TestSpecialRequestProcessing(unittest.TestCase):
    ...
    def test_process_dishframed_worker_request_handles_timeout(self):
        state = bridge.State()
        client = FakeTelegramClient()
        config = make_config()
        ...
        special_request_processing.process_dishframed_worker_request(
            request,
            process_dishframed_request_fn=mock.Mock(
                side_effect=subprocess.TimeoutExpired(cmd=["dishframed"], timeout=30)
            ),
            send_timeout_response_fn=send_timeout_response,
            send_canceled_response_fn=mock.Mock(),
            emit_worker_exception_and_reply_fn=mock.Mock(),
        ...
        send_timeout_response.assert_called_once_with(client, config, 1, 404, 80)

    def test_process_dishframed_worker_request_handles_cancel_and_generic_error(self):
        state = bridge.State()
        client = FakeTelegramClient()
        config = make_config()
        request = bridge_handlers.build_dishframed_request(
        ...
        send_canceled_response = mock.Mock()

        special_request_processing.process_dishframed_worker_request(
            request,
            process_dishframed_request_fn=mock.Mock(
                side_effect=special_request_processing.ExecutorCancelledError("stop")
            ),
            send_timeout_response_fn=mock.Mock(),
        ...
        send_canceled_response.assert_called_once_with(client, 1, 405, 81)

        emit_worker_exception_and_reply = mock.Mock()
        special_request_processing.process_dishframed_worker_request(
            request,
            process_dishframed_request_fn=mock.Mock(side_effect=RuntimeError("boom")),
            send_timeout_response_fn=mock.Mock(),
            send_canceled_response_fn=mock.Mock(),
            emit_worker_exception_and_reply_fn=emit_worker_exception_and_reply,
        )
        ...
        emit_worker_exception_and_reply.assert_called_once_with(
            ...
            event_fields={"chat_id": 1, "message_id": 405, "phase": "dishframed"},
            client=client,
            config=config,
            chat_id=1,
            message_id=405,
