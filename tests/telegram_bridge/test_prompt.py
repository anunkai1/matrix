...
class TestPrompt(unittest.TestCase):
    ...
    def test_process_prompt_request_passes_steered_follow_up_count_to_goal_hook(self):
        state = bridge.State()
        client = FakeTelegramClient()
        config = make_config()
        request = bridge_handlers.build_prompt_request(
        ...
        result = bridge_executor.attach_cached_executor_result(
            result,
            "thread-1",
            "hello",
            steered_follow_up_count=2,
        )
        ...
        runtime = bridge_prompt_execution.build_prompt_execution_runtime(
            ...
            execute_prompt_with_retry_fn=lambda **_kwargs: result,
            finalize_prompt_success_fn=lambda **_kwargs: ("thread-1", "hello"),
            finalize_request_progress_fn=lambda **_kwargs: None,
            emit_event_fn=lambda *args, **kwargs: None,
        )

        with mock.patch.object(bridge_prompt_execution.goal_loop, "maybe_handle_goal_post_turn") as goal_hook:
            bridge_prompt_execution.process_prompt_request(request, runtime=runtime)
